...
auto LCD::fill_screen(uint16_t color) const -> void { draw_rectangle({0, 0}, width, height, color); }

auto LCD::clear_screen() const -> void { fill_screen(WHITE); }

auto LCD::draw_horizontal_line(pixel_location_t pos, uint16_t h, uint16_t color) const -> void { draw_rectangle(pos, width, h, color); }
...
auto LCD::draw_bitmap(pixel_location_t pos, uint16_t w, uint16_t h, std::vector<uint8_t> const& bitmap) const -> void {
    if ((pos.x >= width) || (pos.y >= height)) return;
    if ((pos.x + w - 1) >= width) w = width - pos.x; // if our rectangle extends past the horizontal dimensions of the screen
    if ((pos.y + h - 1) >= height) h = height - pos.y;
    set_addr_window(pos.x, pos.y, pos.x + w - 1, pos.y + h - 1);
        ...
        for (int8_t j = 0; j < 8; j++) {
            if (line & 0x1) {
                if (size == 1) // default size
                    ...
                    draw_rectangle({pos.x + (i * size), pos.y + (j * size)}, (size + pos.x + (i * size)) / 10, (size + 1 + pos.y + (j * size)) / 10,
                                   color);
...
void LCD::drawPixel(pixel_location_t pos, uint16_t color) { draw_rectangle(pos, 1, 1, color); }

void LCD::drawPixelSize(pixel_location_t pos, uint16_t color, uint16_t size) { draw_rectangle(pos, size, size, color); }
