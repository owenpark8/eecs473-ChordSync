...
void ILI9488_Init(void) {
  ...
  writecommand(0x3A);  // Interface Pixel Format //changed to 3A.
...
void spiwrite(uint8_t data) { HAL_SPI_Transmit(&hspi1, &data, 1, 1); }
...
/*void ILI9488::commandList(uint8_t *addr)
 {
        ...
        while (numCommands--) {
                ...
                if (ms) {
                        ms = pgm_read_byte(addr++); // Read post-command delay
time (ms) if (ms == 255) ms = 500;     // If 255, delay for 500 ms delay(ms);
                }
        }
...
void ILI9341_Draw_Colour_Burst(uint16_t Colour, uint32_t Size) {
  ...
  if (Remainder_from_block > 0) {
    SPI1_TX_completed_flag = 0;
    HAL_SPI_Transmit_DMA(&hspi1, (unsigned char *)burst_buffer,
                         Remainder_from_block);
    while (SPI1_TX_completed_flag == 0)
      ;
  }
  HAL_GPIO_WritePin(TFT_CS_GPIO_Port, TFT_CS_Pin, GPIO_PIN_SET);
...
void ILI9341_Fill_Screen1(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                          uint16_t Colour) {
  //	LCD_Address_Set(xsta,ysta+OFFSET_Y,xend-1,yend-1+OFFSET_Y);/
...
// }
//
// void ILI9341FilledRectangle(uint16_t x, uint16_t y, uint16_t width, uint16_t
// height, colour_t colour)
//{
...
//	}
//
//	setAddrWindow(x, y, x + ILI9488_TFTWIDTH - 1U, y + ILI9488_TFTHEIGHT -
// 1U); 	totalBytesToWrite = (uint32_t)width * (uint32_t)height *
//(uint32_t)sizeof(colour_t); 	bytesToWriteThisTime = DMA_BUFFER_SIZE *
//(uint16_t)sizeof(colour_t);
//
...
// }
//
// static void filledRectangleClip(int16_t x, int16_t y, uint16_t width,
// uint16_t height, colour_t colour)
//{
//	int16_t x_end;
//	int16_t y_end;
//
//	if (x > (int16_t)ILI9488_TFTWIDTH - 1 || y > (int16_t)ILI9488_TFTHEIGHT
//- 1)
//	{
//		return;
//	}
...
//{
//	filledRectangleClip(0U, 0U, (uint16_t)ILI9488_TFTWIDTH,
//(uint16_t)ILI9488_TFTHEIGHT, colour);
// }
//
